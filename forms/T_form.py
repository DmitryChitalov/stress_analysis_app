...
class T_form_class(QWidget):
        ...
        if self.con.open():
            if 'patches' in self.con.tables():
                ...
                for el_p in self.patches_lst:
                    ...
                    self.type_list = ["symmetryPlane", "tractionDisplacement", "empty", "zeroGradient"]
    ...
    def type_on_change(self, bvc):
        ...
        elif type_txt == "symmetryPlane" or type_txt == "empty" or type_txt == "zeroGradient":
    ...
    def on_btnSave_clicked(self):
        # создаем таблицу T
        if 'T' not in self.par.con.tables():
            query = QtSql.QSqlQuery()
            query.exec("CREATE TABLE T(param, value)")
            ...
            query.exec("INSERT INTO T(param, value) VALUES ('%s','%s')" % ('dimensions', dimensions_str))
            query.exec("INSERT INTO T(param, value) VALUES ('%s','%s')" % ('internalField', internalField_str))

            # записываем файл T
            if os.path.exists(self.full_dir + '/0/T'):
                os.remove(self.full_dir + '/0/T')
		
            shutil.copyfile("./matches/Shablon/0/T", self.full_dir + '/0/T')

            T = open(self.full_dir + '/0/T', 'a')
            ...
            T.write(dimensions_bl + internalField_bl + boundaryField_bl_start)

            i = 0
            for el_p in self.patches_lst:
                if self.type_list_main[i].currentText() == "symmetryPlane" or self.type_list_main[i].currentText() == "empty" or self.type_list_main[i].currentText() == "zeroGradient":
                    type_txt = self.main_grid_list[i].itemAtPosition(0, 1).widget().currentText()
                    query.exec("INSERT INTO T(param, value) VALUES ('%s','%s')" % (el_p, type_txt))
                    ...
                    query.exec("INSERT INTO T(param, value) VALUES ('%s','%s')" % (el_p, complex_str))
            ...
            boundaryField_bl_start = '}' + '\n\n'
            T.write(boundaryField_bl_start)
        ...
        if 'T' in self.par.con.tables():
            ...
            query.exec("REPLACE INTO T(param, value) VALUES ('%s','%s')" % ('dimensions', dimensions_str))
            query.exec("REPLACE INTO T(param, value) VALUES ('%s','%s')" % ('internalField', internalField_str))
            ...
            for el_p in self.patches_lst:
                if self.type_list_main[i].currentText() == "symmetryPlane" or self.type_list_main[i].currentText() == "empty" or self.type_list_main[i].currentText() == "zeroGradient":
                    type_txt = self.main_grid_list[i].itemAtPosition(0, 1).widget().currentText()
                    query.exec("REPLACE INTO T(param, value) VALUES ('%s','%s')" % (el_p, type_txt))
                    ...
                    query.exec("REPLACE INTO T(param, value) VALUES ('%s','%s')" % (el_p, complex_str))
        ...
        outf = open(self.full_dir + '/0/T')

        if self.interface_lng_val == 'Russian':
            msg_lbl = QLabel(
                '<span style="color:green">Файл T сохранен</span>')
        elif self.interface_lng_val == 'English':
            msg_lbl = QLabel(
                '<span style="color:green">The T file was saved</span>')
        ...
        if self.interface_lng_val == 'Russian':
            self.par.outf_lbl.setText("Файл " + "<font color='peru'>" + 'T' + "</font>")
        elif self.interface_lng_val == 'English':
            self.par.outf_lbl.setText("<font color='peru'>" + 'T' + "</font>" + " file")
