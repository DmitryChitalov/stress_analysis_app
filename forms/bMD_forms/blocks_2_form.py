	...
	def out_frame_func(int_lng, prj_path, mesh_name_txt, blocks_visible): 
		...
		if os.path.exists(blocks_1_path_file):
			...
			for el in obj_list:
				...
				el_srya_lbl = QLabel(srya_list[h])
				...
				if srya_list[h] == "simpleGrading" and mg_list[h] == True:
					if napr_list[h] == 'x':
						...
						sg_mg_grid = QGridLayout()
						sg_mg_lbl = QLabel(srya_list[h])
			...
			blocks_2_btnSave = QPushButton("Записать")
			if int_lng == 'Russian':
